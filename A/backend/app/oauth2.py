...
def create_access_token(data: dict): # used in routers/auth.py/login()
    to_encode = data.copy() # {"user_id": 'alan@gmail.com'}
    expire = datetime.utcnow() + timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
    to_encode.update({"exp": expire})   # {"user_id": 'alan@gmail.com', "exp": expire-datetime}
...
def verify_access_token(token: str, credentials_exception):
    try:
        ...
        token_data = schemas.TokenData(id=id)   # simply stores the user_id(emailid)
    except JWTError:
...
def get_current_user(token: str = Depends(oauth2_scheme), db: Session = Depends(database.get_db)):
    ...
    token = verify_access_token(token, credentials_exception)   # simply stores the user_id(emailid)
